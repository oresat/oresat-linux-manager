...
static void
EmergencyRxCallback(const uint16_t ident, const uint16_t errorCode,
                    const uint8_t errorRegister, const uint8_t errorBit,
                    const uint32_t infoCode) {
    int16_t nodeIdRx = ident ? (ident & 0x7F) : CO_activeNodeId;

    log_printf(LOG_NOTICE, DBG_EMERGENCY_RX, nodeIdRx, errorCode, errorRegister,
               errorBit, infoCode);
}
#endif
...
static void
HeartbeatNmtChangedCallback(uint8_t nodeId, uint8_t idx,
                            CO_NMT_internalState_t state, void *object) {
    (void)object;
    log_printf(LOG_NOTICE, DBG_HB_CONS_NMT_CHANGE, nodeId, idx,
               NmtState2Str(state), state);
}
#endif
...
printUsage(char *progName) {
    ...
    printf("  -i <Interface>       CANbus interface (can0, vcan0, etc).\n");
    printf("  -n <Node ID>         CANopen Node-id (1..127).\n");
    printf("  -p <RT priority>     Real-time priority of RT thread (1 .. 99). "
           "If not set or\n"
           "                       set to -1, then normal scheduler is used "
           "for RT thread.\n");
    printf("  -r                   Enable reboot on CANopen NMT reset_node "
           "command. \n");
    printf("  -d                   Run the process as a daemon.\n");
    printf("  -v                   Enable verbose logging.\n");
    printf("  -c                   Enable CPU frequency control.\n");
}
...
main(int argc, char *argv[]) {
    ...
    if (!daemon_flag)
        openlog(argv[0], LOG_PID | LOG_PERROR,
                LOG_USER); /* print also to standard error */
    ...
    if (rtPriority != -1
        && (rtPriority < sched_get_priority_min(SCHED_FIFO)
    ...
    // make all the dirs
    if (mkdir_path(FREAD_CACHE_DIR, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP
                                        | S_IXGRP | S_IROTH | S_IXOTH)
        != 0)
        log_printf(LOG_CRIT, "failed to make fread cache dir " FREAD_CACHE_DIR);
    if (mkdir_path(FWRITE_CACHE_DIR, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP
                                         | S_IXGRP | S_IROTH | S_IXOTH)
        != 0)
        log_printf(LOG_CRIT,
                   "failed to make fwrite cache dir " FWRITE_CACHE_DIR);
    if (mkdir_path(FREAD_TMP_DIR, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP
                                      | S_IXGRP | S_IROTH | S_IXOTH)
        != 0)
        log_printf(LOG_CRIT, "failed to make fread tmp dir " FREAD_TMP_DIR);
    if (mkdir_path(FWRITE_TMP_DIR, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP
                                       | S_IXGRP | S_IROTH | S_IXOTH)
        != 0)
    ...
    CO_fstream_t CO_fread_data
        = CO_FSTREAM_INITALIZER(FREAD_TMP_DIR, fread_cache);
    CO_fstream_t CO_fwrite_data
    ...
    for (unsigned int i = 0; i < CO_NO_TPDO; i++) {
        if ((OD_TPDOCommunicationParameter[i].COB_IDUsedByTPDO & 0xFFF)
            == 0x180 + (0x100 * (i % 4)))
            OD_TPDOCommunicationParameter[i].COB_IDUsedByTPDO
                += configs.node_id + i / 4;
    }
    for (unsigned int i = 0; i < CO_NO_RPDO; i++) {
        if ((OD_RPDOCommunicationParameter[i].COB_IDUsedByRPDO & 0xFFF)
            == 0x200 + (0x100 * (i % 4)))
            OD_RPDOCommunicationParameter[i].COB_IDUsedByRPDO
                += configs.node_id + i / 4;
    }
    ...
    while (reset != CO_RESET_APP && reset != CO_RESET_QUIT
        ...
        if (!CO->nodeIdUnconfigured) {
...
#if (CO_CONFIG_TRACE) & CO_CONFIG_TRACE_ENABLE
            /* Initialize time */
            CO_time_init(&CO_time, CO->SDO[0], &OD_time.epochTimeBaseMs,
                         &OD_time.epochTimeOffsetMs, 0x2130);
#endif

            for (int i = 1; i < CO_NO_TPDO; ++i)
                CO->TPDO[i]->valid = false;
#ifdef TIME_PRODUCER
            CO->TPDO[CO_NO_TPDO - 1]->valid
                = true; // gps last tpdo is the time sync TPDO
#endif
            ...
            CO_OD_configure(CO->SDO[0], OD_3003_fread, CO_fread_ODF,
                            &CO_fread_data, 0, 0U);
            CO_OD_configure(CO->SDO[0], OD_3004_fwrite, CO_fwrite_ODF,
                            &CO_fwrite_data, 0, 0U);
...
make_daemon(const char *pid_file) {
    ...
    if (freopen("/dev/null", "r", stdin) == NULL) {
        log_printf(LOG_ERR,
                   "Error: Failed to redirect streams to /dev/null!\n");
        exit(EXIT_FAILURE);
    }
